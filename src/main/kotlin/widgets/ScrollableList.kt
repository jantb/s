...
class ScrollableList(
    ...
    private fun updateResults() {
        ...
        val updateResult = UpdateResult(offset = indexOffset, length = length)
        Channels.cmdChannel.put(updateResult)
    }
    private fun updateResults(result: List<Domain>,query :String) {
        this.query = query
        val length = if (!::maxCharBounds.isInitialized || maxCharBounds.height.toInt() == 0) {
            0
        } else {
